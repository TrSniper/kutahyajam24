...
public class Door : MonoBehaviour, IInteractableWithDirection
{
    ...
    void OpenDoor(Vector3 lookDir)
    {
        if (isOpen && (Vector3.Dot(transform.localPosition.normalized,lookDir.normalized)<0)) // looking to the same general direction as door's zed axis
        {
            doorTransform.DOLocalRotate(new Vector3(0, 0, 0), 1f).OnComplete(() => isOpen = false);
            Debug.Log("isOpen && alpha < 0");
        }
        else if (!isOpen && (Vector3.Dot(transform.localPosition.normalized, lookDir.normalized) > 0)) // the other way
        {
            doorTransform.DOLocalRotate(new Vector3(0, 90, 0), 1f).OnComplete(() => isOpen = true);
            Debug.Log("!isOpen && alpha > 0");
        }
